...
export default function queryApi(WrappedComponent, mapPropsToOptions) {
  return class ApiQuery extends Component {
    ...
    dispatch(props, callback) {
      ...
      this.context.store.dispatch(({ api }) => {
        const ref = api.child(options.path);
        return {
...
export const queryApiServer = renderAppCallback => {
  serverFetching = true;
